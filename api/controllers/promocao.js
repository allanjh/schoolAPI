...
module.exports.getPromocoes = function (app, req, res) {
    ...
    promocoes.getPromocoes(function (error, result) {
        if (error) {
            ...
            if (result == '') {
                result = {error: "Nenhuma Promoção válida encontrada!"};
...
//Exporta a função que insere um produto ou vários produtos em uma nova promoção 
//no banco de dados
module.exports.insertProdutoPromocao = function (app, req, res) {
    ...
    /* Verifica as informações passadas
    req.assert('idpromocao', 'ID do supermercado é obrigatório').notEmpty();
    req.assert('idpromocao', 'ID da promoção deve ser um Número').isInt();
    req.assert('idproduto', 'ID do produto é obrigatório').notEmpty();
    req.assert('idproduto', 'ID do supermercado deve ser um Número').isInt();
    req.assert('preco', 'o campo data inicio promoção é obrigatório').notEmpty();
    ...
    */
    ...
    // Acessa o método de inserir uma nova promoção esperando o retorno de um possível
    // erro ou resultado da inserção
    promocoes.insertProdutoPromocao(novaPromocao, function (error, result) {
        if (error) {
            res.status(400).json(error);
        } else {
            res.json(result);
